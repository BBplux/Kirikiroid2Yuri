...
int CDVDAudioCodecFFmpeg::Decode(uint8_t* pData, int iSize, double dts, double pts)
{
  ...
  ret = avcodec_receive_frame(m_pCodecContext, m_pFrame1);
  if (ret < 0 && ret != AVERROR_EOF)
      return ret;
  m_gotFrame = 1;

  if (m_pFrame1->channels > 0 && m_pFrame1->channel_layout == 0)
      m_pFrame1->channel_layout = av_get_default_channel_layout(m_pFrame1->channels);
  else if (m_pFrame1->channels == 0 && m_pFrame1->channel_layout > 0)
      m_pFrame1->channels = av_get_channel_layout_nb_channels(m_pFrame1->channel_layout);
